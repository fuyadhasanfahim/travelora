...
import { IconChevronRight, IconStarFilled, IconMapPin } from "@tabler/icons-react";

const TITLE = "California Sunset/Twilight Boat Cruise";
  ...
  return (
    <section className="relative -mt-20 w-full">
      <div className="relative h-[380px] w-full overflow-hidden sm:h-[420px] lg:h-[450px]">
        ...
        >
          <Image src="/images/tours/td-hero.jpg" alt={TITLE} fill priority sizes="100vw" className="object-cover" />
        </motion.div>
          ...
          >
            <span className="flex items-center gap-1.5 text-sm">
              <span className="flex items-center gap-0.5 text-primary">
                {Array.from({ length: 4 }).map((_, i) => <IconStarFilled key={i} className="size-4" />)}
                <IconStarFilled className="size-4 text-primary/40" />
              </span>
              4.6 (496 Reviews)
            </span>
            <span className="flex items-center gap-1.5 text-sm">
