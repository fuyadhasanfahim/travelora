...
/* ────────────────────────────────────────────────────────────────────────
 * Shared filter panel used by the desktop sidebar and the mobile drawer.
 * URL search params are the single source of truth — same logic as before.
 * ────────────────────────────────────────────────────────────────────── */
  ...
  return (
    <div className="border-b border-black/[0.08] py-5 last:border-0">
      ...
      >
        {title}
        <IconChevronDown
          className={`size-5 text-ink/50 transition-transform ${open ? "" : "-rotate-90"}`}
          stroke={2}
        />
      </button>
...
export function useActiveFilterCount() {
  ...
  if (sp.get("category")) n += sp.get("category")!.split(",").filter(Boolean).length;
  if (sp.get("destination")) n += sp.get("destination")!.split(",").filter(Boolean).length;
  if (sp.get("minRating")) n += 1;
...
export function useClearAllFilters() {
  const router = useRouter();
  return () => router.push("/tours", { scroll: false });
}

/* ────────────────────────────────────────────────────────────────────────
 * The panel itself — no outer card chrome so it can sit inside either the
 * desktop card or the mobile drawer.
 * ────────────────────────────────────────────────────────────────────── */

export default function TourFiltersPanel() {
  ...
  const [price, setPrice] = useState<number>(
    maxPriceParam ? Number(maxPriceParam) : PRICE_MAX,
  );
  useEffect(() => {
  ...
  return (
    <>
          ...
          {categoryCounts.map((c) => {
            ...
            return (
              <div key={c.label} className="flex items-center justify-between">
                <label className="group flex cursor-pointer items-center gap-3 py-1.5 text-[#515151] transition-colors hover:text-navy">
                  <input
                    type="checkbox"
                    checked={checked}
      ...
      {/* Popular Destination */}
      <Section title="Popular Destination" defaultOpen={false}>
        <div className="space-y-1">
          {POPULAR_DESTINATIONS.map((d) => {
            const key = d.split(",")[0].trim();
            const checked = dests.includes(key.toLowerCase());
            return (
              <label key={d} className="group flex cursor-pointer items-center gap-3 py-1.5 text-[#515151] transition-colors hover:text-navy">
                <input
                  type="checkbox"
                  checked={checked}
      ...
      <Section title="Rating">
        <div className="space-y-1">
          {RATINGS.map((r) => {
            ...
            return (
              <label key={r} className="group flex cursor-pointer items-center gap-3 py-1.5">
                <input
                  type="radio"
                  name="minRating"
                  checked={active}
